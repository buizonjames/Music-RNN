...
def showdata(audiobeats, duration=None, offset=None, beatfinder=None, device=None, showpred=True):
    ...
    #with open('D:/Thesis/librosaPred.txt', 'r') as f:
    #    pred_beats2 = f.read().splitlines()

    #with open('D:/Thesis/madmomPred.txt', 'r') as f:
    #    pred_beats3 = f.read().splitlines()
    ...
    if audiobeats.beats_file:
        ...
        plt.ylim(0, 2)
    if showpred:
        plt.vlines(pred_beats, 1, 2, color='b', label='Librosa beat prediction')
        plt.vlines(onsets_selected_times, 0, 1,  color='m', linestyles='-', alpha=1, label='Model beat prediction')
    else:
